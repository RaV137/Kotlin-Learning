package hyperskill.challenging.seamcarving.stage6
...
data class Arguments(
    ...
    val widthScaling: Int,
    val heightScaling: Int,
)

object ArgumentParser {
    private const val IN_ARG_NAME = "-in"
    private const val OUT_ARG_NAME = "-out"
    private const val WIDTH_ARG_NAME = "-width"
    private const val HEIGHT_ARG_NAME = "-height"
    ...
    fun parseArguments(args: Array<String>): Arguments {
        ...
        val widthArgIdx = args.indexOf(WIDTH_ARG_NAME)
        val heightArgIdx = args.indexOf(HEIGHT_ARG_NAME)

        if (inArgIdx >= 0 && outArgIdx >= 0 && widthArgIdx >= 0 && heightArgIdx >= 0) {
            val inputFilename = args[inArgIdx + 1]
            val outputFilename = args[outArgIdx + 1]
            val widthScaling = args[widthArgIdx + 1].toInt()
            val heightScaling = args[heightArgIdx + 1].toInt()
            return Arguments(inputFilename, outputFilename, widthScaling, heightScaling)
        }
...
fun main(args: Array<String>) {
    ...
    val afterVerticalScalingImage = performVerticalSeamCarving(image, arguments.widthScaling)
    val afterHorizontalScalingImage = performHorizontalSeamCarving(afterVerticalScalingImage, arguments.heightScaling)
    saveImageToFile(afterHorizontalScalingImage, arguments.outputFilename)
}
...
fun performVerticalSeamCarving(image: BufferedImage, times: Int) =
    performSeamCarving(image, times)

fun performHorizontalSeamCarving(image: BufferedImage, times: Int): BufferedImage {
    val transposedImage = transposeImage(image)
    val newImage = performSeamCarving(transposedImage, times)
    return transposeImage(newImage)
}

fun performSeamCarving(image: BufferedImage, times: Int): BufferedImage {
    var newImage = image
    repeat(times) {
//        if (it % 10 == 0) {
//            saveImageToFile(newImage, "tmp/after-$it.png")
//        }

        val energyMatrix = createEnergyMatrix(newImage)
        createSeams(energyMatrix)
        markMinimumSeam(energyMatrix)
        newImage = removeSeamFromImage(newImage, energyMatrix)
    }
    return newImage
}
...
fun removeSeamFromImage(image: BufferedImage, energyMatrix: Array<Array<EnergyPixel>>): BufferedImage {
    val newWidth = image.width - 1
    val newImage = BufferedImage(newWidth, image.height, image.type)

    for (row in 0 until image.height) {
        var oldImageCol = 0
        for (col in 0 until newImage.width) {
            if (!energyMatrix[row][oldImageCol].minimumSeamMember) {
                newImage.setRGB(col, row, image.getRGB(oldImageCol, row))
            } else {
                newImage.setRGB(col, row, image.getRGB(++oldImageCol, row))
            }
            oldImageCol++
