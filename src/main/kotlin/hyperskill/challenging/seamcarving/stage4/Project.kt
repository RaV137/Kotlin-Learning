package hyperskill.challenging.seamcarving.stage4
...
fun main(args: Array<String>) {
    ...
    drawSeamOnImage(image, energyMatrix)
...
fun createSeams(energyMatrix: Array<Array<EnergyPixel>>) {
    for (row in 0 until energyMatrix.lastIndex) {
        for (col in energyMatrix[0].indices) {
...
fun markMinimumSeam(energyMatrix: Array<Array<EnergyPixel>>) {
    ...
    for (row in energyMatrix.lastIndex downTo 0) {
        ...
        for (col in range) {
            if (currRow[col].totalEnergy < minEnergy) {
                minEnergy = currRow[col].totalEnergy
                minEnergyIndex = col
            }
        }
