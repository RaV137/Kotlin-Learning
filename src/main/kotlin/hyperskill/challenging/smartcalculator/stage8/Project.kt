...
enum class Operator(val calculation: (BigInteger, BigInteger) -> BigInteger) {
    PLUS({ a, b -> a + b }),
    MINUS({ a, b -> a - b }),
    MULTIPLY({ a, b -> a * b }),
    DIVIDE({ a, b -> a / b }),
    ...
    while (true) {
        ...
        try {
            when {
                ...
                input.startsWith("/") -> resolveCommand(input)
...
fun resolveCommand(input: String) {
    when (input) {
        "/help" -> printHelp()
...
fun resolveLiteral(input: String): BigInteger {
    return try {
        BigInteger(input)
...
fun resolveVariable(input: String): BigInteger {
...
fun resolveExpression(input: String): BigInteger {
    try {
...
fun calculateFromPostfix(postfixList: List<String>): BigInteger {
    val numberStack = Stack<BigInteger>()
